...
def plt_mc_2(X_train, y_train, classes, centers, std):
    css = np.unique(y_train)
    fig, ax = plt.subplots(1, 1, figsize=(3, 3))
    fig.canvas.toolbar_visible = False
    fig.canvas.header_visible = False
...
def plt_layer_relu(X, Y, W1, b1, classes):
    ...
    for i in range(nunits):
        def layerf(x): return np.maximum(0, (np.dot(x, W1[:, i]) + b1[i]))
        plt_prob_z(ax[i], layerf)
        plt_mc_data(ax[i], X, Y, classes, map=dkcolors_map,
                    legend=True, size=50, m='o')
...
def plt_output_layer_linear(X, Y, W, b, classes, x0_rng=None, x1_rng=None):
    ...
    for i, axi in enumerate(ax.flat):
        def layerf(x): return np.dot(x, W[:, i]) + b[i]
        plt_prob_z(axi, layerf, x0_rng=x0_rng, x1_rng=x1_rng)
        plt_mc_data(axi, X, Y, classes, map=dkcolors_map,
                    legend=True, size=50, m='o')
